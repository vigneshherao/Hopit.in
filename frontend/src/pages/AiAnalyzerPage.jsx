...
export function AiAnalyzerPage() {
  ...
  const isWorkerFitMode = user?.role === 'worker' || searchParams.get('mode') === 'worker-fit';
  const form = useForm({
    resolver: zodResolver(schema),
    defaultValues: {
      soilType: searchParams.get('soilType') || undefined,
      landArea: searchParams.get('landArea') || undefined,
      areaUnit: searchParams.get('areaUnit') || undefined,
      state: searchParams.get('state') || undefined,
      district: searchParams.get('district') || undefined,
      season: searchParams.get('season') || 'monsoon',
      waterAvailability: searchParams.get('waterAvailability') || 'adequate',
      budget: 100000,
      farmingExperience: isWorkerFitMode ? 'experienced' : 'beginner',
      preferredFarmingType: isWorkerFitMode ? 'mixed' : 'organic',
      irrigationAvailable: getBooleanParam(searchParams, 'irrigationAvailable', true),
      roadAccess: getBooleanParam(searchParams, 'roadAccess', true),
      ownerParticipation: getBooleanParam(searchParams, 'ownerParticipation', false),
      marketDistanceKm: searchParams.get('marketDistanceKm') || undefined,
  ...
  async function onSubmit(values) {
    ...
    try {
      const result = await landAnalysis.mutateAsync({
        ...
        notes: values.notes || (isWorkerFitMode ? 'Analyze this land for farm worker fit, skill match, likely labour demand, work duration, safety risks, and recommended worker preparation.' : undefined),
      });
  ...
  return (
    <section className="page-shell space-y-6">
      <div className="relative overflow-hidden rounded-[36px] border border-emerald-100 bg-white p-7 shadow-xl shadow-emerald-900/5">
        ...
        <p className="text-sm font-semibold uppercase text-emerald-600">{isWorkerFitMode ? 'AI Work Suitability' : 'AI Land Analyzer'}</p>
        <h1 className="mt-2 max-w-3xl text-4xl font-semibold text-slate-950">
          {isWorkerFitMode ? 'Check which lands match your farm skills and work goals.' : 'Turn land conditions into crop and profit recommendations.'}
        </h1>
        <p className="mt-3 max-w-2xl text-muted-foreground">
          {isWorkerFitMode
            ? 'Use visible land details to understand work season, labour demand, water constraints, safety risks, and the best way to approach the owner.'
            : 'Select one of your Hopt It lands or enter land details manually. Recommendations are generated only by the backend AI provider.'}
        </p>
      </div>
        ...
        <Card>
          ...
          <CardContent>
            <form className="space-y-5" onSubmit={form.handleSubmit(onSubmit)}>
              {step === 0 ? <LandStep lands={lands} selectedLand={selectedLand} form={form} userRole={user?.role} isWorkerFitMode={isWorkerFitMode} /> : null}
              ...
              {step === 4 ? <PreferenceStep form={form} isWorkerFitMode={isWorkerFitMode} /> : null}
              ...
              {landAnalysis.isPending ? <ScanningStage stage={analysisStages[stageIndex]} isWorkerFitMode={isWorkerFitMode} /> : null}
...
function LandStep({ lands, selectedLand, form, userRole, isWorkerFitMode }) {
  const canManageLands = ['owner', 'admin'].includes(userRole);

  return (
    <div className="space-y-4">
      {isWorkerFitMode ? (
        <div className="rounded-3xl border border-emerald-100 bg-emerald-50 p-4 text-sm text-emerald-950">
          Worker mode uses manual/public land details. It does not access private owner documents or hidden land data.
        </div>
      ) : null}
      <label className="grid gap-2 text-sm font-medium">Select your land<select className="premium-select" {...form.register('landId')} disabled={isWorkerFitMode}><option value="">Manual entry</option>{lands.map((land) => <option key={land._id} value={land._id}>{land.title} · {land.location?.district}</option>)}</select></label>
      ...
      {canManageLands ? <Button asChild variant="outline"><Link to="/my-lands">Manage lands</Link></Button> : null}
    </div>
  );
}
...
function PreferenceStep({ form, isWorkerFitMode }) {
  return (
    <div className="grid gap-4">
      <SelectField label={isWorkerFitMode ? 'Preferred work environment' : 'Preferred farming type'} name="preferredFarmingType" options={farmingTypes} form={form} />
      <Field label={isWorkerFitMode ? 'Your crop or work skills comma separated' : 'Preferred crops comma separated'} name="preferredCrops" form={form} />
      <Field label={isWorkerFitMode ? 'Worker fit notes' : 'Analysis notes'} name="notes" form={form} />
    </div>
  );
}
...
function getBooleanParam(searchParams, key, fallback) {
