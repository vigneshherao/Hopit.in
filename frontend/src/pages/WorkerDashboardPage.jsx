...
import { CardGridSkeleton, CardSkeleton, Skeleton } from '@/components/ui/feedback.jsx';
...
export function WorkerDashboardPage() {
  ...
  const metricsLoading = profileQuery.isLoading || applicationsQuery.isLoading || bookingsQuery.isLoading;
    ...
    <section className="page-shell space-y-6">
      <div className="flex flex-col justify-between gap-4 rounded-[32px] border border-emerald-100 bg-white/90 p-6 shadow-sm lg:flex-row lg:items-end">
        <div>
          ...
          {profileQuery.isLoading ? <Skeleton className="mt-3 h-9 w-full max-w-xl" /> : <h1 className="mt-2 text-3xl font-semibold text-slate-950">{profile?.headline ?? 'Build your agriculture work profile'}</h1>}
      ...
      {metricsLoading ? <CardGridSkeleton /> : <div className="grid gap-4 md:grid-cols-4">
      ...
      <div className="grid gap-5 lg:grid-cols-[1fr_1.3fr]">
        {profileQuery.isLoading ? <CardSkeleton lines={4} /> : <Card>
        ...
        {bookingsQuery.isLoading ? <CardSkeleton lines={4} /> : <Card>
